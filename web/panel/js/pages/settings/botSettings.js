...
$(function () {
    socket.getDBValues('get_tts_settings', {
        ...
        keys: ['ttsVoice', 'ttsVolume', 'ttsRate', 'ttsPitch']
    }, true, function (e) {
        // Update ttsVoice.
        let voiceNames = window.speechSynthesis.getVoices().map((voice) => voice.name);

        $('#tts')
            .append(helpers.getDropdownGroup('tts-voices', 'TTS Voice', e.ttsVoice, voiceNames, 'Audio hook to be played when the command is ran.'))
            // Update ttsVolume.
        ...
        switch (false) {
            ...
            default:
                socket.updateDBValues('update_tts_settings', {
                    tables: ['ttsSettings', 'ttsSettings', 'ttsSettings', 'ttsSettings'],
                    keys: ['ttsVoice', 'ttsVolume', 'ttsRate', 'ttsPitch'],
                    values: [ttsVoice, ttsVolume.val(), ttsRate.val(), ttsPitch.val()]
                }, function () {
